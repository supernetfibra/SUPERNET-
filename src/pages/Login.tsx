 ...
 * and progressive disclosure (CPF first, then password).
 */
...
export default function Login() {
  ...
  const [multiContactData, setMultiContactData] = useState<any>(null);
  ...
  const handleCpfSubmit = (e: FormEvent) => {
    ...
    if (normalized.length !== 11) {
      return; // Invalid CPF, form won't submit via HTML5
    }
  ...
  const handlePasswordSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (result?.hasMultipleContacts && result?.contacts?.length > 0) {
        ...
        // Authentication complete, go to dashboard
        navigate("/dashboard");
      }
    } catch {
